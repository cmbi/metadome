...
def generate_pfam_alignment_mappings(pfam_id):
    ...
    if not len (hmmeralign_output) == 0:
        ...
        _log.debug("Creating the mappings for '"+str(n_instances) +"' '"+pfam_id+"' domain occurrences based on the HMM alignment to consensus and original domain sequence")
        ...
        for uniprot_ac in consensus_pos_per_protein.keys():
            for uniprot_pos in consensus_pos_per_protein[uniprot_ac].keys():
                ...
                mappings = [x for x in protein_mappings[uniprot_acs_to_ids[uniprot_ac]] if x.uniprot_position == uniprot_pos]
